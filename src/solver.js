...
function getNextNodes(n, visited, required) {
  ...
  candidates.sort(function (a, b) {
    return required.has(b) - required.has(a);
  });
...
function isOuterpoint(n) {
  return (
    n.x == 0 || n.y == 0 || n.x == puzzle.width - 1 || n.y == puzzle.height - 1
  );
}
...
function getLeftRight(cur, next) {
  ...
  if (next.x > cur.x) {
    return [point(cur.x, cur.y - 1), point(cur.x, cur.y)];
  } else if (next.x < cur.x) {
    return [point(next.x, next.y), point(next.x, next.y - 1)];
  }

  if (next.y > cur.y) {
    return [point(cur.x, cur.y), point(cur.x - 1, cur.y)];
  } else if (next.y < cur.y) {
    return [point(next.x - 1, next.y), point(next.x, next.y)];
  }
}
...
function separateAreasStep(last, cur, areas, segment) {
  ...
  if (isOuterpoint(last)) {
    ...
    if (segment.length <= 1 && !innerEdge) {
      ...
      // Run segregation and for the second to last area,
...
function getNonCancellationCellsInRegion(area) {
  ...
  for (var c of area) {
    if (
      puzzle.cells[c.x][c.y].type != CELL_TYPE.NONE &&
      puzzle.cells[c.x][c.y].type != CELL_TYPE.CANCELLATION
    ) {
      ret.push(c);
    }
  }
...
function determineAuxilaryRequired() {
  ...
  for (var x = 0; x < puzzle.width - 1; x++) {
    for (var y = 0; y < puzzle.height - 1; y++) {
      ...
      if (
        x < puzzle.width - 2 &&
        !colorsCompatible(puzzle.cells[x][y], puzzle.cells[x + 1][y])
      ) {
        aux.add(point(x + 1, y));
      ...
      if (
        y < puzzle.height - 2 &&
        !colorsCompatible(puzzle.cells[x][y], puzzle.cells[x][y + 1])
      ) {
        aux.add(point(x, y + 1));
        aux.add(point(x + 1, y + 1));
      }
...
function findSolution(
  ...
  if (!path || path.length == 0) {
    ...
    if (puzzle.nodes[cn.x][cn.y].type == NODE_TYPE.EXIT) {
      if (
        checkLastArea(prevn, cn, areas, segment) &&
        checkRequiredNodes(path, required) &&
        checkRequiredEdges(path, edgeRequired)
      ) {
        return path;
      } else {
        exitsRemaining--;
